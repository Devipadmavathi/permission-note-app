...
@section('content')
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 py-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg sm:rounded-xl p-6">
            <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-6">Edit User: {{ $user->name }}</h2>
            ...
            <form method="POST" action="{{ route('users.update', $user->id) }}">
                ...
                <!-- Password (optional) -->
                <div class="mb-4">
                    <label for="password" class="block font-medium text-sm text-gray-700 dark:text-gray-300">New Password (leave blank to keep current)</label>
                ...
                <div class="mb-6">
                    ...
                    <select multiple name="permissions[]" id="permissions"
                        ...
                        @foreach($permissions as $permission)
                            <option value="{{ $permission->name }}" {{ in_array($permission->name, old('permissions', $userPermissions)) ? 'selected' : '' }}>
                                {{ $permission->name }}
                            </option>
                        @endforeach
                    </select>
